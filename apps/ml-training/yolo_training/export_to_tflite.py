...
def export_savedmodel(onnx_path: str, output_path: str = None) -> Optional[str]:
    ...
    try:
        # Use tf2onnx to convert ONNX to SavedModel
        import tf2onnx

        # Convert ONNX to SavedModel
        tf2onnx.convert.from_onnx(
            onnx_path, output_path=output_path, opset=13, output_frozen_graph=False
        )
        ...
        print("[yellow]tf2onnx not available, trying alternative method[/yellow]")
        return _export_savedmodel_alternative(onnx_path, output_path)
    except Exception as e:
        print(f"[red]SavedModel export error: {e}[/red]")
        return None


def _export_savedmodel_alternative(onnx_path: str, output_path: str) -> Optional[str]:
    """Alternative method to convert ONNX to SavedModel using onnx-tf."""
    try:
        ...
        if os.path.exists(output_path):
            print(f"[green]SavedModel export successful (alternative): {output_path}[/green]")
            return output_path
        else:
            print("[red]SavedModel export failed (alternative)[/red]")
            return None

    except Exception as e:
        print(f"[red]Alternative SavedModel export failed: {e}[/red]")
        return None
...
def export_pipeline(weights_path: str, output_dir: str = None) -> Optional[str]:
    """
    Complete export pipeline: YOLO -> ONNX -> SavedModel -> TFLite.
    ...
    Returns:
        Path to final TFLite model or None if failed
    """
    if output_dir is None:
        output_dir = str(Path(weights_path).parent)

    print(f"[bold cyan]Starting complete export pipeline for {weights_path}[/bold cyan]")

    # Step 1: Export to ONNX
    onnx_path = export_onnx(
        weights_path, os.path.join(output_dir, f"{Path(weights_path).stem}.onnx")
    )
    if not onnx_path:
        return None

    # Step 2: Convert ONNX to SavedModel
    savedmodel_path = export_savedmodel(
        onnx_path, os.path.join(output_dir, f"{Path(weights_path).stem}_savedmodel")
    )
    ...
    tflite_path = export_tflite(
        savedmodel_path, os.path.join(output_dir, "model_fp16.tflite"), quantize=True
    )
    if not tflite_path:
        return None

    # Step 4: Validate TFLite model
    if validate_tflite_model(tflite_path):
        print(f"[bold green]Export pipeline completed successfully![/bold green]")
        print(f"[green]Final TFLite model: {tflite_path}[/green]")
        return tflite_path
    else:
        print("[red]TFLite validation failed[/red]")
        return None
