...
def split_dataset(
    ...
    print(
        f"Split dataset: {len(train_files)} train, {len(val_files)} val, {len(test_files)} test images"
    )
...
def main() -> None:
    ...
    if args.convert_csv:
        ...
        results = run_yolo_autolabel(
            args.input, args.output_csv, args.model, args.limit, args.confidence_threshold
        )
        if results:
