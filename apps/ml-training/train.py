...
def parse_args():
    ...
    p.add_argument(
        ...
        type=int,
        default=150,
        help="Representative sample count for int8 quantization",
    )
    return p.parse_args()
...
def build_datasets(merged_dir: str, cfg: TrainConfig):
    ...
    # Enhanced Augmentation for waste classification
...
def build_model(cfg: TrainConfig, class_names: list[str]):
    ...
    if cfg.fine_tune_from is not None:
        ...
        lr = 5e-6  # Even lower LR for fine-tuning
    else:
    ...
    x = keras.layers.Dense(256, activation="relu")(x)  # Add dense layer
...
def main():
    ...
    model.save(h5_path)
    model.save(os.path.join(cfg.model_dir, "waste_classifier"))
    ...
    print("Training complete. Run export_tflite.py for TFLite conversion.")
