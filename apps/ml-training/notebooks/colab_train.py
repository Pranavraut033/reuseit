#!/usr/bin/env python
"""Train waste classification model using Kaggle datasets on Google Colab.
...
# Upload kaggle.json (from Kaggle account settings)
...
# Optional: Mount Google Drive for saving models
...
def build_datasets(merged_dir: str, cfg: TrainConfig):
    ...
    args = parse_args()  # Get args for augmentation config
    ...
    # Create datasets with AUTOTUNE
    train_ds = tf.data.Dataset.from_tensor_slices((train_files, train_labels))
    train_ds = train_ds.map(load_train_image, num_parallel_calls=tf.data.AUTOTUNE)
    train_ds = train_ds.shuffle(
        buffer_size=len(train_files), seed=seed
    )  # Shuffle buffer > dataset size
    train_ds = train_ds.batch(batch_size).prefetch(tf.data.AUTOTUNE)

    val_ds = tf.data.Dataset.from_tensor_slices((val_files, val_labels))
    val_ds = val_ds.map(load_val_image, num_parallel_calls=tf.data.AUTOTUNE)
    val_ds = val_ds.batch(batch_size).prefetch(tf.data.AUTOTUNE)

    test_ds = tf.data.Dataset.from_tensor_slices((test_files, test_labels))
    test_ds = test_ds.map(load_val_image, num_parallel_calls=tf.data.AUTOTUNE)
...
def build_model(cfg: TrainConfig, class_names: list[str]):
    ...
    if cfg.fine_tune_from is not None:
        ...
        lr = 1e-3  # Increased for better initial learning
    else:
