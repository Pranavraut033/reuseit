...
const PostItem: React.FC<PostItemProps> = ({ post, onSwipeRefresh }) => {
        ...
        <View className="mb-3 flex-row items-center">
          {post.author.avatarUrl && (
            <View className="mr-3 relative">
              <Image source={{ uri: post.author.avatarUrl }} className="h-10 w-10 rounded-full border-2 border-gray-200" />
              {/* small badge */}
            ...
            <View>
              ...
              >
                {post.images.map((img, idx) => (
                  <Image key={idx} source={{ uri: img }} className="h-56 w-full" resizeMode="cover" />
                ))}
              </ScrollView>
      ...
      <View className="flex-row items-center justify-between pt-2 border-t border-gray-100">
        <Row className="items-center" space={16}>
          <TouchableOpacity onPress={handleLike} className="items-center">
            <Row space={4}>
              <Ionicons name={state.liked || liking ? 'heart' : 'heart-outline'} size={20} color={state.liked || liking ? '#EF4444' : '#6B7280'} />
              <Text className="text-gray-600 text-sm">{state.likes}</Text>
            </Row>
          </TouchableOpacity>

          <TouchableOpacity onPress={(e) => { e.stopPropagation(); router.push(`/posts/${post.id}`); }} className="items-center">
            <Row space={4}>
